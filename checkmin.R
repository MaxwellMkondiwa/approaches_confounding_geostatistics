...
print(i)
   ...
   out = xtable(tab, digits=-3, math.style.exponents = T, display = c('s','s','s','g','g','g','s'))
	...
	print(tab %>% filter(n == 1000) %>% select(bias) %>% abs() %>% min(na.rm=T))
	print('\n')
print(tab %>% filter(n == 1000) %>% select(mse) %>% abs() %>% min(na.rm=T))
        print('\n')
print(tab %>% filter(n == 10000) %>% select(bias) %>% abs() %>% min(na.rm=T))
        print('\n')
print(tab %>% filter(n == 10000) %>% select(mse) %>% abs() %>% min(na.rm=T))
        print('\n')
